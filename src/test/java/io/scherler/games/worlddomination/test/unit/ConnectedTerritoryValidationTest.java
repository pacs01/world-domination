...
import io.scherler.games.worlddomination.test.unit.testdata.Game;
...
class ConnectedTerritoryValidationTest {
    ...
    @BeforeEach
    void setUp() {
        territories = game.getOccupations().stream()
            .filter(o -> o.getPlayer().getId().equals(game.getPlayerOne().getId()))
            .map(OccupationEntity::getTerritory).collect(Collectors.toList());
    }

    @Test
    void testConnectedTerritories() {
        Assertions.assertTrue(territoryService
            .areConnected(world.getAlaska(), world.getEasternUnitedStates(), territories));
        Assertions.assertTrue(territoryService
            .areConnected(world.getWesternUnitedStates(), world.getIceland(), territories));
    }

    @Test
    void testNonConnectedTerritories() {
        Assertions.assertFalse(territoryService
            .areConnected(world.getAlaska(), world.getWesternEurope(), territories));
        Assertions.assertFalse(
            territoryService.areConnected(world.getPeru(), world.getIceland(), territories));
        Assertions.assertFalse(
            territoryService.areConnected(world.getArgentina(), world.getIceland(), territories));
    }

    @Test
    void testConnectedWithSameTerritories() {
        Assertions.assertTrue(
            territoryService.areConnected(world.getAlaska(), world.getAlaska(), territories));
        Assertions.assertFalse(
            territoryService.areConnected(world.getArgentina(), world.getArgentina(), territories));
    }
}
